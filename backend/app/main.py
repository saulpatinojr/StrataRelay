...
@app.post("/analyze")
async def analyze_data(data: dict = Body(...)):
    try:
        file_type = data.get('fileType')
        raw_sheets = data.get('rawSheets', {})

        if not file_type or not raw_sheets:
            raise HTTPException(status_code=400, detail="Invalid data structure. 'fileType' and 'rawSheets' are required.")

        # The CloudAssessmentEngine expects pandas DataFrames.
        # We will convert the raw sheet data from JSON to DataFrames.
        dataframes = {sheet_name: pd.DataFrame(sheet_data) for sheet_name, sheet_data in raw_sheets.items()}

        # --- This is where the magic happens ---
        # We need to call the correct analysis function based on the file type.
        if file_type == 'rvtools':
            vinfo_df = dataframes.get('vInfo')
            vcpu_df = dataframes.get('vCPU')
            vmemory_df = dataframes.get('vMemory')
            vdisk_df = dataframes.get('vDisk')
            ...
            analysis_result = assessment_engine.analyze_rvtools_data(
                df_vinfo=vinfo_df,
                df_vcpu=vcpu_df,
                df_vmemory=vmemory_df,
                df_vdisk=vdisk_df
            )
        elif file_type == 'azmigrate':
            # The current assessment engine doesn't have a dedicated azmigrate function.
            # This is a key area for improvement. For now, we'll return a placeholder.
            # In the next phase, I will build out the azmigrate analysis logic.
            analysis_result = {"message": "Azure Migrate analysis coming soon.", "data": dataframes.keys()}
        else:
